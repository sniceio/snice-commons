 ...
 */
public abstract class BaseBuffer implements OldBuffer {
    protected static final byte LF = '\n';
    protected static final byte CR = '\r';

    @Override
    public abstract OldBuffer clone();
     ...
     */
    @Override
    public final OldBuffer readUntil(final byte b) throws IOException, ByteNotFoundException {
     ...
     */
    @Override
    public final OldBuffer readUntil(final int maxBytes, final byte... bytes) throws IOException, ByteNotFoundException,
            IllegalArgumentException {
        final OldBuffer result = readUntilSafe(maxBytes, bytes);
        if (result == null) {
     ...
     */
    @Override
    public final OldBuffer readUntilSafe(final int maxBytes, final byte... bytes) throws IOException, IllegalArgumentException {
     ...
     */
    @Override
    public final OldBuffer readLine() throws IOException {
        final int start = getReaderIndex();
     ...
     */
    @Override
    public final OldBuffer readUntilSingleCRLF() throws IOException {
        final int start = getReaderIndex();
        int found = 0;
    ...
    @Override
    public final OldBuffer readUntilDoubleCRLF() throws IOException {
        final int start = getReaderIndex();
        int found = 0;
    ...
    public final int parseToInt(final int radix) throws NumberFormatException, IOException {
        if (getReadableBytes() == 0) {
            throw new NumberFormatException("OldBuffer is empty, cannot convert it to an integer");
        }
