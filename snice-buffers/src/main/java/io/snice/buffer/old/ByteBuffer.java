...
public class ByteBuffer extends AbstractBuffer {
     ...
     */
    @Override
    public OldBuffer readBytes(final int length) throws IndexOutOfBoundsException {
        if (length == 0) {
            return Buffers.EMPTY_BUFFER;
        }
     ...
     * TODO: should probably also have this one return a T extends OldBuffer or something
     *
     * @param readerIndex
     * @param lowerBoundary
     * @param upperBoundary
     ...
     * @return
     */
    protected OldBuffer createBuffer(final int readerIndex, final int lowerBoundary, final int upperBoundary,
                                     final int writerIndex, final byte[] buffer) {
    ...
    public long getUnsignedInt(final int index) {
        ...
        checkIndex(i);
        checkIndex(i + 3);
        return OldBuffer.unsignedInt(buffer[i], buffer[i + 1], buffer[i + 2], buffer[i + 3]);
    }
    ...
    @Override
    public void getBytes(final int index, final OldBuffer dst) {
        if (index < 0) {
