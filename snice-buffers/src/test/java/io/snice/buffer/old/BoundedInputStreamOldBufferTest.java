...
import io.snice.buffer.old.BoundedInputStreamBuffer;
import io.snice.buffer.old.Buffers;
import io.snice.buffer.old.InputStreamBuffer;
 ...
 */
public class BoundedInputStreamOldBufferTest {
    ...
    private void verifyDynamicallyIncreaseBuffer(final int initialCapacity) throws Exception {
        ...
        final OldBuffer buffer = new InputStreamBuffer(initialCapacity, new ByteArrayInputStream(content));

        final OldBuffer initial = buffer.readBytes(50);
        assertContent(initial, content, 0, 50);

        final OldBuffer forty = buffer.readBytes(40);
        assertContent(forty, content, 50, 40);

        final OldBuffer fifty = buffer.readBytes(50);
        assertContent(fifty, content, 90, 50);

        // we are now at 140. Read another 300 bytes,
        final OldBuffer threehundred = buffer.readBytes(300);
        assertContent(threehundred, content, 140, 300);

        // which leaves us with 65 bytes. read those as well and check them
        final OldBuffer theRest = buffer.readBytes(65);
    ...
    public void testReadingOnDemand() throws Exception {
        ...
        final OldBuffer outer = Buffers.wrap(in);
        final OldBuffer first = outer.readBytes(50);
        final OldBuffer second = outer.readBytes(150);
     ...
     */
    private void assertContent(final OldBuffer buffer, final byte[] actual, final int offset, final int length) throws Exception {
    ...
    public void testSliceAndWrapAroundBuffer() throws Exception {
        ...
        assertTrue("OldBuffer size not valid", bufferSize > content.length && content.length * 2 > bufferSize);
...
//            final OldBuffer slice = buffer.slice((long)buffer.getReaderIndex(), buffer.getReaderIndex() + content.length);
//            assertContent(slice, content, 0, content.length);

            final OldBuffer data = buffer.readBytes(content.length);
